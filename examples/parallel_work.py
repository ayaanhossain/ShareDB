...
def stream_tasks(inDB_path, num_task, num_proc):
    ...
    inDB = ShareDB(
        ...
        map_size=5 * 10**8,
    )  # And we estimate to require ~500MB for results
...
def para_conv(inDB_path, outDB_path, exec_id, num_task, num_proc):
    ...
    outDB = ShareDB(
        ...
        compress=True,  # Serialized msgpack-ed lists are further compressed
        map_size=5 * 10**8 // num_proc,
    )  # And we split total allocation uniformly
    ...
    for X, Y in inDB.multiget(key_iter):
        ...
        # Actual execution
        result = tuple(
            int(v) for v in np.convolve(X, Y)
        )  # Compute and store result in a list
...
def main():
    ...
    # Fire task streamer
    task_streamer = Process(target=stream_tasks, args=(inDB_path, num_task, num_proc))
    task_streamer.start()
    ...
    for exec_id in range(num_proc):
        ...
        task_executor = Process(
            target=para_conv, args=(inDB_path, outDB_path, exec_id, num_task, num_proc)
        )
